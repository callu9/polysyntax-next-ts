...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return getArticleMetadata(post, absoluteUrl(origin, localePath('en', '/blog/react-reconciliation')), await getLocaleAlternates(origin, '/blog/react-reconciliation'));
}
...
export default async function ArticleLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <>
      <ArticleStructuredData post={post} canonical={absoluteUrl(await getSiteOrigin(), localePath('en', '/blog/react-reconciliation'))} />
      {children}
    </>
  );
}
